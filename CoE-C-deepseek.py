...
def compute_CoE_C(token_index, step_length, hidden_states):
    ...
    for j in range(layer_num):
        all_pos_hs = np.array([np.array(hidden_states[pos][j][0][0].cpu()) for pos in range(token_index, token_index + step_length)])
        hs_all_layer.append(np.mean(all_pos_hs, axis=0))
    
    # 计算归一化分母：最后一层与第一层的欧几里得距离
...
def Self_Certainty_Selection(dataset, config: Config, model, tokenizer, device, N=4, save_results=False, lambda_weight=0.5):
    ...
    if save_results:
        ...
        output_file = f"./TTT_data/Best_of_{N}_Transformers_Step_Certainty_lambda_{lambda_weight}_deepseek_normalized_multiply.json"
