"""
python CoE-C-deepseek.py --subset_size 10 --lambda_weight 0.5 --model_path /root/autodl-tmp/data/models/modelscope_cache/models/deepseek-ai/deepseek-llm-7b-chat
python CoE-C-deepseek.py --lambda_weight 0.0 --model_path /root/autodl-tmp/data/models/modelscope_cache/models/deepseek-ai/deepseek-llm-7b-chat
"""
...
def compute_CoE_C(token_index, step_length, hidden_states):
    ...
    for j in range(layer_num):
        ...
        for pos in range(token_index, token_index + step_length):
            # 根據生成步驟 (pos) 處理不同的 hidden_states 形狀
            if pos == 0:
                # 步驟 0 : 張量形狀為 (1, prompt_len, hidden_size)
                tensor = hidden_states[0][j]
                # 使用 [0, -1] 來選取第1個batch的最後一個token
                vector = tensor[0, -1] 
            else:
                # 步驟 > 0 : 張量形狀為 (1, 1, hidden_size)
                # 我們提取那唯一的 token 的隱藏狀態
                tensor = hidden_states[pos][j]
                # 使用 [0, 0] 來選取第0個batch的第一個（也是唯一的）token
                vector = tensor[0, 0]
            # ===============================================================
...
def calculate_step_confidence_with_full_probs(prompt_ids, response_ids, logits, hidden_states, tokenizer, lambda_weight=0.5):
    """
    根据公式计算步骤置信度（使用完整概率分布）
    使用完整概率分布计算每个token的置信度，然后对步骤内所有token取平均
...
def generate_with_transformers(model, tokenizer, prompt, device, temperature=0.7, max_tokens=512, num_return_sequences=1):
    ...
    Returns:
        list: 生成的响应列表，每个包含对应序列的tokens, logits, 和 hidden_states
    """
    # 编码输入
    ...
    # 提取生成的序列和对应的logits/hidden_states
    generated_sequences = []
    for i in range(num_return_sequences):
        ...
        # 重新运行模型以获取完整logits (根据您的要求，暂时保留这部分逻辑)
        with torch.no_grad():
        ...
        # outputs.hidden_states 是 (生成步数, 模型层数) 的嵌套元组
        # 最内层的张量形状为 (num_return_sequences, seq_len, hidden_size)
        # 我们需要对每个张量进行切片，只取出第 i 个序列的数据
        
        single_sequence_hidden_states = tuple(
            tuple(
                # 对每个张量进行切片 layer_tensor[i] 或 layer_tensor[i:i+1]
                # 使用 i:i+1 可以保持维度为 (1, seq_len, hidden_size)，是更好的实践
        ...
        generated_sequences.append({
            ...
            # 使用切片后、只属于当前序列的 hidden_states
            "hidden_states": single_sequence_hidden_states, 
        })
...
def Self_Certainty_Selection(dataset, config: Config, model, tokenizer, device, N=4, save_results=False, lambda_weight=0.5):
    ...
    if save_results:
        ...
        output_file = f"./TTT_data/Best_of_{N}_Transformers_Step_COE-C_lambda_{lambda_weight}_deepseek_multiply.json"
